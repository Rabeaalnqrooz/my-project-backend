...
export const register = asyncHandler(async (req, res) => {
  ...
  // 🛠️ الفحص الذكي: هل خاصية تعطيل الإيميلات مفعلة؟
  const isEmailDisabled = process.env.DISABLE_EMAIL_SENDING === "true";

  // إنشاء المستخدم مع وضع حالة التفعيل بناءً على متغير البيئة
  const user = await User.create({
    firstName,
    lastName,
    email,
    password,
    isVerified: isEmailDisabled ? true : false, // يتفعل تلقائياً إذا كان الإرسال معطلاً
  });

  if (isEmailDisabled) {
    // 🎉 الحساب مفعل وجاهز فوراً دون الحاجة لإرسال أي شيء!
    res.status(201).json({
      success: true,
      message: "تم إنشاء وتفعيل الحساب بنجاح وتخطي تأكيد البريد الإلكتروني!",
    });
  } else {
    // ✉️ الحالة الطبيعية: توليد رابط التحقق وإرساله بالخلفية دون تعطيل السيرفر
    const rawToken = user.generateEmailVerifyToken();
    await user.save({ validateBeforeSave: false });
...
export const resendVerification = asyncHandler(async (req, res) => {
  ...
  // حماية إضافية: إذا كانت الخدمة معطلة، لا تسمح بإعادة الإرسال
  if (process.env.DISABLE_EMAIL_SENDING === "true") {
    res.status(400);
    throw new Error("ميزة تأكيد البريد الإلكتروني معطلة حالياً.");
  }
...
export const forgotPassword = asyncHandler(async (req, res) => {
  ...
  // حماية إضافية: إذا كانت الخدمة معطلة، لا تسمح بطلب استعادة الباسورد عبر الإيميل
  if (process.env.DISABLE_EMAIL_SENDING === "true") {
    res.status(400);
    throw new Error(
      "ميزة استعادة كلمة المرور معطلة حالياً أثناء بيئة التطوير.",
    );
  }
...
export const resendResetOTP = asyncHandler(async (req, res) => {
  ...
  if (process.env.DISABLE_EMAIL_SENDING === "true") {
    res.status(400);
    throw new Error("ميزة إرسال كود التحقق معطلة حالياً.");
  }
